...
def vector_force_field(azim, pc, steer_cmd):
    ...
    w = 0.8
    car_vec[0] = w * (steer_cmd) + (1 - w) * obstacle_vec[0]
...
class UTURN:
    ...
    def stopit(self,azim,dis ):
        ...
        for i in range(cN):
            if   -0.7 < cY[i] < 0.7 and 1 < cX[i] < 3.5 and self.state == 0:
                print("stopped")
                self.state = 1
...
class STATIC :
    ...
    def __init__(self) :
        ...
        self.start_cnt             =   0
        self.end_cnt               =   0
        self.end_limit : int       =   200

        self.send_static_flag: int

    # org cmd : steer, velocity
    def static_obs(self, azim, dist) :
        
        cluster_num, drum_num = 0, 0
        
        cluster_num = self.clustering(azim, dist)
        ...
        if self.static_mission_flag and self.end_cnt < self.end_limit :
            ...
            steer_cmd = vector_force_field(azim, dist, 0)
            
            self.send_static_flag = YEAH_TUNNEL_STATIC

            self.check_termination(drum_num)

        send_data(self.sm_send_flag,  (1,1) , [self.send_static_flag])
        send_data(self.sm_send_steer, (1,1) , [steer_cmd])

    def check_termination(self, drum_num) :
        ...
        elif self.end_cnt > self.end_limit: 
    ...
    def clustering(self, azim, dist) :
        ...
        if stack.shape[0] > 0:
            ...
            # 9.21 추가 for debugging
            for Idx in range(cluster_num) :

                cluster_x_pos = np.mean(np.array(filtered_x_pos)[np.array(labels) == Idx])
                cluster_y_pos = np.mean(np.array(filtered_y_pos)[np.array(labels) == Idx])

                if abs(cluster_x_pos) < roi_height and abs(cluster_y_pos) < roi_width :

                    cluster_x.append(cluster_x_pos)
                    cluster_y.append(cluster_y_pos)
                    
        # plt.cla()
        # plt.plot(x_pos, y_pos, 'ro', markersize = 3)
        # plt.plot(filtered_x_pos, filtered_y_pos, 'bo', markersize = 3)
        # plt.plot(cluster_x, cluster_y, 'ro', markersize = 5)
        # plt.grid(True)
        # plt.pause(0.001)
    ...
    def check_static(self, cluster_num, drum_num) :
        ...
        if cluster_num >= 2 and drum_num >= 1 : self.start_cnt += 1
            
        if self.start_cnt >= 5 : self.static_mission_flag = True
    
        # print("\n-----------------------------------------------------")
        # print(f"cluster : {cluster_num}     drum : {drum_num}")
    ...
    def yaw_control(self, yaw) :
        ...
        if abs(error - prev_error < 50) : error = prev_error
