...
class TargetCourse:
    ...
    def get_crosstrack_err(self,px,py):
        a1,a2 = self.cx[self.my_curr_index] , self.cy[self.my_curr_index]
        tx,ty = self.cx[self.old_nearest_point_index] , self.cy[self.old_nearest_point_index]
        A = [tx -a1,ty-a2]
        B = [tx -px, ty - py]
        # in frenet frame 
        oejuck = A[0] * B[1] - A[1]*B[0]
        cross_err = np.sign(oejuck) * np.hypot(px-a1,py-a2)
        return cross_err
...
def impact_Angle_steer_control(px, py, yaw, tx, ty):
    ...
    gamma =  (90 - yaw) * np.pi / 180
    px = px + 0.9 * math.cos(gamma)
    py = py + 0.9 * math.sin(gamma)
    los = math.atan2(ty - py, tx - px)
    # alpha = -(math.atan2(ty - py, tx - px) - yaw * np.pi /180)
...
def calc_cross_err(x,y,yaw,cx,cy):
    L = 1.4
    # Calc front axle position
    gam = 90 - yaw
    fx = x + L * np.cos(gam * np.pi /180)
    fy = y + L * np.sin(gam * np.pi /180)
    ...
    front_axle_vec = [-np.cos(gam + np.pi / 2),
                      -np.sin(gam + np.pi / 2)]
    error_front_axle = np.dot([dx[target_idx], dy[target_idx]], front_axle_vec)
    return target_idx, error_front_axle

def stanley_control( yaw,v, cyaw,target_idx,  error_front_axle):
    """
    ...
    theta_e = (90 - cyaw[target_idx]*180/np.pi )  -yaw
    # theta_d corrects the cross track error
    theta_d = np.arctan2(k * error_front_axle, 10) 
    # Steering control
    delta =  theta_d
...
class LateralController():
    def __init__(self):
        ...
        self.d_err = 0
        self.prev_err = 0
    def con(self,crosstrack_err):
        self.err = crosstrack_err
        self.i_err += crosstrack_err * self.Ts 
        self.d_err = (crosstrack_err -self.prev_err) / self.Ts
        self.i_err = min(max(-20, self.i_err),20)
        steer_cmd = self.Kp * crosstrack_err #+ self.Ki * self.i_err + self.Kd * self.d_err
        steer_cmd = min(max(steer_cmd , -28) , 28)
        
        self.prev_err =crosstrack_err
        return steer_cmd
