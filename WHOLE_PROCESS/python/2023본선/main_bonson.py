...
def bonplot(waymaker: WayMaker):
    ...
    # plt.xlim((x-10 ,x+10))
    # plt.ylim((y-10 ,y+10))
    # dd = [90 - i *  180/np.pi for i in Wa.cyaw]
    # plt.plot(dd)
    plt.grid()
    plt.pause(0.0001)
    # ----------------------------------PLOT------------------------------------#   
...
if __name__ == "__main__":
    ...
    while not (isEnd()):
        ...
        #     cmd_gear, cmd_velocity, cmd_steer, cmd_brake = deliv.delivery_mission(mission_flag, 0.8, [ref_cmd_gear, ref_cmd_velocity, cmd_steer, ref_cmd_brake])
        #     cmd_velocity = 6
        # elif mission_flag == MISSION_DELIVERY_B:
        #     Wa.Trajectory.set_Lf(2.4)
        #     cmd_gear, cmd_velocity, cmd_steer, cmd_brake = deliv.delivery_mission(mission_flag, -0.3,[ref_cmd_gear, ref_cmd_velocity, cmd_steer, ref_cmd_brake])
        #     cmd_velocity = 6
        if mission_flag == MISSION_STATIC:
            cmd_velocity = 7
            Wa.Trajectory.set_Lf(3.2)
        ...
        elif mission_flag == MISSION_TURNLEFT3:
            if cam_brake_flag == CAM2MAIN_BRAKE:
                cmd_velocity = 0
                cmd_gear = 1
                cmd_brake = 100
        ...
        elif mission_flag == MISSION_TURNLEFT4:
            if cam_brake_flag == CAM2MAIN_BRAKE:
                cmd_velocity = 0
                cmd_gear = 1
                cmd_brake = 100
            ...
            Wa.Trajectory.set_Lf(4.0)
            cmd_gear, cmd_velocity, cmd_steer, cmd_brake = para.parallel_parking(x, y, lat, lon, yaw, azim, dist,
                                                                                 [cmd_gear, cmd_velocity, cmd_steer,
                                                                                  cmd_brake])
            # pass
