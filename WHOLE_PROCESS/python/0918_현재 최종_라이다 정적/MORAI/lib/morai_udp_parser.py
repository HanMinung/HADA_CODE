    ...
    def data_parsing(self, raw_data):
        ...
        if self.data_type == 'erp_status':
            ...
            # if header == '#MoraiInfo$' and data_length[0] == 132:
                ...
                overhang, wheelbase, rear_overhang = struct.unpack(
                    'fff', raw_data[57:69])
                ...
                accel_x, accel_y, accel_z = struct.unpack(
                    'fff', raw_data[105:117])
            ...
            if header == '#MoraiObjInfo$':
                ...
                for i in range(20):
                    start_byte = i*68
                    obj_id, obj_type = struct.unpack(
                        'hh', raw_data[start_byte+offset_byte:start_byte+offset_byte+4])
                    pos_x, pos_y, pos_z = struct.unpack(
                        'fff', raw_data[start_byte+offset_byte+4:start_byte+offset_byte+16])
                    heading = struct.unpack(
                        'f', raw_data[start_byte+offset_byte+16:start_byte+offset_byte+20])[0]
                    size_x, size_y, size_z = struct.unpack(
                        'fff', raw_data[start_byte+offset_byte+20:start_byte+offset_byte+32])
                    overhang, wheelbase, rear_overhang = struct.unpack(
                        'fff', raw_data[start_byte+offset_byte+32:start_byte+offset_byte+44])
                    vel_x, vel_y, vel_z = struct.unpack(
                        'fff', raw_data[start_byte+offset_byte+44:start_byte+offset_byte+56])
                    accel_x, accel_y, accel_z = struct.unpack(
                        'fff', raw_data[start_byte+offset_byte+56:start_byte+offset_byte+68])

                    obj_info_list = [obj_id, obj_type, pos_x, pos_y, pos_z, heading, size_x, size_y, size_z,
                                     overhang, wheelbase, rear_overhang, vel_x, vel_y, vel_z, accel_x, accel_y, accel_z]
            ...
            if header == '#TrafficLight$' and data_length[0] == 16:
                ...
                traffic_type, traffic_status = struct.unpack(
                    '2h', raw_data[42:46])
                self.parsed_data = [traffic_index,
                                    traffic_type, traffic_status]
...
class udp_sender:
    ...
    def send_data(self, data):
        if self.data_type == 'erp_ctrl_cmd':
            ...
            lower = packed_mode + packed_gear + packed_cmd_type + packed_velocity + \
                packed_acceleration + packed_accel + packed_brake + packed_steering_angle
            send_data = self.upper + lower + self.tail

        elif self.data_type == 'set_traffic':
            # data=[trafficlightindex, trafficlightstatus]
