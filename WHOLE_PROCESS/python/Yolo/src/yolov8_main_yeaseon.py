...
if __name__ == '__main__':
    ...
    '''
    # 카메라
    cap = camera_init(bottom=True)


    yolo_line = YOLOV8(camera_capture=cap['bottom'],
                       cam_mode='line', save_output=True,
                       window_name='bot_line')

    yolo_rubber = YOLOV8(camera_capture=cap['bottom'],
                           cam_mode='rubber', save_output=True,
                           window_name='bot_rub')
    ...
    while True:
        ...
        frame = yolo_rubber.get_frame()
        if frame.size != 0:
            cv.imshow("rubber", frame)
