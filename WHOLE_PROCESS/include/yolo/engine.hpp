...
#ifndef HADA_ENGINE_HPP
#define HADA_ENGINE_HPP
#pragma once
...
public:
	...
	resizeKeepAspectRatioPadRightBottom(const cv::cuda::GpuMat &input, size_t height, size_t width,
	...
	static void
	transformOutput(std::vector<std::vector<std::vector<float>>> &input, std::vector<std::vector<float>> &output);
	...
	static cv::cuda::GpuMat
	blobFromGpuMats(const std::vector<cv::cuda::GpuMat> &batchInput, const std::array<float, 3> &subVals,
