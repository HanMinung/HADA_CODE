...
def clustering():#X array Y array of Lidar
    ...
    cluster = DBSCAN(eps=0.4,min_samples=14).fit(X)    
...
def generate_path(candid_x, candid_y):
    ...
    if len(candid_x) >0:
        ...
        for i in candid_indices:
            if i not in path_indices and vector_angle(e_x ,candid[i] - startpoint ) < 70 * np.pi / 180:
        ...
        if cN >3:
            try:
                ...
                for i in range(0, len(simps)):
                    ...
                    path = generate_path(path_candidate_x,path_candidate_y)
                plt.cla()
                plt.triplot(points[:,0], points[:,1], tri.simplices)
                plt.plot(points[:,0], points[:,1], 'o')
                plt.plot(path_candidate_x,path_candidate_y , 'bo')
