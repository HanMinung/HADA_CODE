...
class DELIVERY :
    ...
    def delivery_mission(self, mission_flag, org_cmd) :
    ...
    def sign_stop(self) :
        ...
        # 배달 구역에서의 속도가 정해지면 deliv_velocity 값은 그에 맞게 수정해야함
        dist_thresh    =  4.0
        deliv_velocity =  6.0

        time_to_brake  =  (dist_thresh + 0.3) / (deliv_velocity * self.unit.KM2MS)
    ...
    def check_restart(self) :
        ...
        if self.is_deliv_brake :
            ...
            if time.time() - self.brake_start_time >= 5.0 :             # 5초 정차 후 출발
