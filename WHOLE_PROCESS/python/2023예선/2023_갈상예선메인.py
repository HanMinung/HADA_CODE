...
if __name__ == "__main__":
    ...
    mission_master = Mission_Gal_YeahSon(Wa.cx, Wa.cy) #for yeason
    ...
    while not (isEnd()):
        ...
        # -----------------------------------------------------------#
        lat, lon, x, y, yaw = data_IO.get_att_data()
        if data_IO.mode == MOD_REAL: meas_speed = math.fabs(data_IO.get_velocity())
        # print(f"Lat: {lat:.6f} Lon: {lon:.6f} Yaw: {yaw:.2f}")
        # if meas_speed < 0.1:
        #     meas_speed = cmd_velocity-3
        ...
        if data_IO.mode == MOD_REAL:
            ...
            # if meas_speed <= 0.01 or meas_speed > 20.0 : meas_speed = cmd_velocity - 2
