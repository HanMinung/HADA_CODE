...
class STATIC :
    ...
    def static_obs(self, azim, dist, yaw, org_cmd) :
        ...
        if self.static_mission_flag and self.end_cnt < 10 :
            
            vel_cmd = 4.0
            
            self.get_ref_yaw(yaw)
            
            yaw_cmd_steer = self.yaw_control(yaw)
            
            # steer_cmd = vector_force_field(azim, dist, yaw_cmd_steer)
            steer_cmd = vector_force_field(azim, dist, 0)
            
            print(f"yaw cmd : {yaw_cmd_steer}       final cmd : {steer_cmd}")
            
            send_data(self.sm_send_flag, (1,1), [YEAH_TUNNEL_STATIC])
    ...
    def check_termination(self, cluster_num) :
        ...
        if cluster_num == 0    :  self.end_cnt += 1

        # print("end_cnt: ", self.end_cnt)
        
        if self.end_cnt >= 10 :  self.static_mission_flag = False
