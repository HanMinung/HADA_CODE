...
class PARALLEL:
    ...
    def __init__(self):
        ...
        # 인지 관련 waypoint
        # K-CITY LAT - LON
        self.recog_reference = [[ 37.239461666666664 , 126.77321500000001 ],
                           [ 37.23941833333333  , 126.77318500000001 ],
                           [ 37.239378333333335 , 126.77316166666667 ]]
        
        # NMH LAT - LON
        # self.park_enter = [[ 36.104242213333336 , 129.38711479666665 ],
    ...
    def make_trajectory(self, longtitude, latitude):
        ...
        parking_heading = -2.64965310947182
        ...
        if self.is_ready_planning:
            ...
            angleP2E = atan2(lon_diff,lat_diff)  # x/y
            ...
            O1_2_Mid = atan2(midpoint_x - O1[1][0], midpoint_y - O1[0][0])
            ...
            # if O2_2_Mid >= np.pi : O2_2_Mid -= 2 * np.pi
                
            # elif O2_2_Mid <= -np.pi : O2_2_Mid += 2 * np.pi
                
            # if O2_2_Stop >= np.pi : O2_2_Stop -= 2 * np.pi
                
            # elif O2_2_Stop <= -np.pi : O2_2_Stop += 2 * np.pi 

            # 얘가 문제다
            theta_1 = np.linspace(O1_2_Mid - alpha * alpha_offset, O1_2_Mid, 4)
            #theta_2 = np.linspace(O2_2_Mid,  O2_2_Mid-alpha * ( 2.2 - alpha_offset ), 4)
            ...
            for Idx in range(4) : 
                X_O1[Idx][0] = O1[1][0] + Rmin * sin(theta_1[Idx]) / self.unit.LON2METER
                Y_O1[Idx][0] = O1[0][0] + Rmin * cos(theta_1[Idx]) / self.unit.LAT2METER
                X_O2[Idx][0] = O2[1][0] - Rmin * sin(theta_2[Idx]) / self.unit.LON2METER 
                Y_O2[Idx][0] = O2[0][0] - Rmin * cos(theta_2[Idx]) / self.unit.LAT2METER
    ...
    def Rimpact_Angle_steer_control(self, lon, lat, yaw):
        ...
        if 5<=self.Guididx:
            if abs(yaw*np.pi/180 - parking_heading) % (2 * pi) <0.25:     #0.32:
                # print("존잘진")
                self.park_cnt += 1
                self.velcmd = 0
                self.brakecmd = 200   
