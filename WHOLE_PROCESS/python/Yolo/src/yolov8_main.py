...
if __name__ == '__main__':
    ...
    # 카메라
    cap = camera_init( top=True)


    yolo_origin = YOLOV8(camera_capture=cap['top'],
                           cam_mode='origin', save_output=True,
                           window_name='top_origin')
    
    yolo_traffic = YOLOV8(camera_capture=cap['top'],
                           cam_mode='traffic', save_output=True,
                           window_name='top_traff')
    
    thread0 = Thread(target=yolo_origin.run)
    thread1 = Thread(target=yolo_traffic.run)
