...
class erpSerial :
    ...
    def send_ctrl_cmd(self,gear_cmd,velocity,steering,braking):
        header="STX".encode()
        tail='\r\n'.encode()
        mode=1
        e_stop=0
        gear=gear_cmd # 0: 전진 1:중립 2:후진
        brake=braking
     
        data=struct.pack('>BBBhhBB',mode,e_stop,gear,int(velocity*10), int(steering*71),brake,self.alive)
