...
class STATIC:
    ...
    def static_obs(self, azim, dist, yaw, org_cmd):
        ...
        print(f"Tunnle mode : {self.send_static_flag}")
    ...
    def check_static(self, cluster_num, drum_num):
        ...
        if cluster_num >= 1 and drum_num >= 2: self.start_cnt += 1

        if self.start_cnt >= 5: self.static_mission_flag = True

        print("\n-----------------------------------------------------")
        print(f"cluster : {cluster_num}     FLAG : {self.static_mission_flag}")
