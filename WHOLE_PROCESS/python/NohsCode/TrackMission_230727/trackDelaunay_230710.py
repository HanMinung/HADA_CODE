...
global distance, azim ,steer_cmd
...
def clustering():#X array Y array of Lidar
    ...
    cluster = DBSCAN(eps=0.45,min_samples=3).fit(X)    
    ...
    for i in range(cluster_num+1):   #클러스터 갯수만큼 반복
        ...
        if math.hypot(center_x ,center_y) < 0.001 :#:or len(points_x) > 21:
            continue
        else:
...
def generate_path(candid_x, candid_y):
    # candid_x = np.array(candid_x)
    # candid_y = np.array(candid_y)
    path            = []   # 경로 점들 저장
    ...
    if len(candid_x) >0:
        ...
        for i in candid_indices:
            if i not in path_indices and vector_angle(e_x ,candid[i] - startpoint ) < 50 * np.pi / 180:
        ...
        for n in range(2):
            min_cost = np.inf
            v1 = candid[next_point_idx] - candid[point_start_idx]
            ang =0
            for i in candid_indices:
    ...
    while (time_stime < time_final) :
        ...
        if cN >3:
            try:
                ...
                plt.plot(path_candidate_x,path_candidate_y , 'yo')
                plt.plot(path[:,0],path[:,1],'k-')

                target_point = path[2]  # 경로 점에서 4번째 점을 목표 지점으로 설정 
                ...
                # print(steer_cmd)
                # For ERP Serial
                command.send_ctrl_cmd(4 , (int)(steer_cmd))
