...
#include "../include/yolo/yolov8.hpp"

YoloV8::YoloV8(const std::string &onnxModelPath, const YoloV8Config &config)
		: PROBABILITY_THRESHOLD(0.3), NMS_THRESHOLD(0.5), TOP_K(50),
		  SEG_CHANNELS(40), SEG_H(192), SEG_W(192),
		  SEGMENTATION_THRESHOLD(0.4), NUM_KPS(config.numKPS),
		  KPS_THRESHOLD(config.kpsThreshold)
{
	...
	if (options.precision == Precision::INT8)
	{
		...
		{
			throw std::runtime_error("Error: Must supply calibration plot path for INT8 calibration");
		}
	}
...
void YoloV8::drawObjectLabels(cv::Mat &image, const std::vector<Object> &objects, unsigned int scale)
{
	...
	for (auto &object: objects)
	{
		...
		cv::putText(image, text, cv::Point(x, y + labelSize.height), cv::FONT_HERSHEY_SIMPLEX, 0.35 * scale, txtColor,
		            scale);
		...
		if (!object.kps.empty())
		{
			...
			for (int k    = 0; k < NUM_KPS + 2; k++)
			{
				if (k < NUM_KPS)
				{
					...
					{
						cv::Scalar kpsColor = cv::Scalar(KPS_COLORS[ k ][ 0 ], KPS_COLORS[ k ][ 1 ],
						                                 KPS_COLORS[ k ][ 2 ]);
				...
				{
					cv::Scalar limbColor = cv::Scalar(LIMB_COLORS[ k ][ 0 ], LIMB_COLORS[ k ][ 1 ],
					                                  LIMB_COLORS[ k ][ 2 ]);
