...
def distinguishLeftRight(rubberX,rubberY):
    ...
    # RightConeX = [] 
    # RightConeY = []
    if len(LstartX) > 0 and _dist(LstartX[0] , LstartY[0],L_cone_x,L_cone_y )<0.5:
        L_cone_x = LstartX[0] 
        L_cone_y = LstartY[0] 
    else :
        L_cone_x = L_cone_x
        L_cone_y = L_cone_y
    if len(RstartX) > 0 and _dist(RstartX[0] , RstartY[0],R_cone_x,R_cone_y )<0.5:
        R_cone_x = RstartX[0] 
        R_cone_y = RstartY[0] 
    else :
        R_cone_x = R_cone_x
...
def clustering():#X array Y array of Lidar
    ...
    for i in range(cluster_num+1):   #클러스터 갯수만큼 반복
        ...
        if math.hypot(center_x ,center_y) < 0.001 :
            continue
        else:
...
def generate_path(candid_x, candid_y):
    ...
    if len(candid_x) >0 :
        ...
        for i in candid_indices:
            if i not in path_indices and vector_angle(e_x ,candid[i] - startpoint ) < 45 * np.pi / 180:
    ...
    while (time_stime < time_final) :
        ...
        if cN >3:
            try:
                ...
                # For ERP Serial
                command.send_ctrl_cmd(5 , (int)(steer_cmd))
