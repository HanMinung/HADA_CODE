...
class erpSerial :
    ...
    def recv_serial_data(self):
        while True :
            ...
            if len(packet)==18:
                ...
                if header == "STX":
                    self.is_data=True
                    self.mode,self.e_stop,self.gear=struct.unpack('BBB',packet[3:6])
    ...
    def send_ctrl_cmd(self,velocity,steering ,brake_cmd):
        header="STX".encode()
        tail='\r\n'.encode()
        mode=1
        e_stop=0
        ...
        data=struct.pack('>BBBhhBB',mode,e_stop,gear,velocity*10,steering*71,brake,self.alive)
